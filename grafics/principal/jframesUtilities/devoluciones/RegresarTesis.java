...
public class RegresarTesis extends JFrame {

    public RegresarTesis() {
        super("Regresar tesis");
    ...
    private JPanel _clearPanelJlist() {
        ...
        nombreLibrosFiltrado = new String[tesis.size()];

        for (int i = 0; i < tesis.size(); i++) {
            if (tesis.get(i).esPrestado()) {
                nombreLibrosFiltrado[i] = tesis.get(i).getTitulo();
            }
        }
    ...
    class EscuchaRegresar implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            String[] nombresLibrosOriginal = new String[tesis.size()];
            nombresLibrosOriginal = UtilJList.sacarNombreTesis(tesis);
            ...
            tesis.get(ward).setPrestado(false);

            try {
                file.guardarDatosEjemplar(new ArrayList<Libro>(), tesis);
                if (tesis.get(ward).esPrestado())
